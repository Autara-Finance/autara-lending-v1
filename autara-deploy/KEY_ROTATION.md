# Key rotation assessment — exposed `keys/` material
...
`53def2dc…1cf64192`, autara-oracle `eee682c2…ed84b15`). The current
**fresh/independent** testnet deploy path
(`autara-deploy/scripts/autara.testnet.fresh.env`) already points every keypair
at the gitignored `autara-deploy/.keys-testnet/` directory and deploys to
brand-new addresses, so it is **not** built on this exposed material. The
exposed `keys/` therefore map to the existing live stage deployment only.
...
| `keys/autara-stage.key` | autara-**program** account keypair; its pubkey **equals** `autara_program::id()` (`53def2dc…`) | program account address | **LOW post-deploy** — the program is already deployed; upgrades are gated by the *deployer/upgrade authority*, not this key. Address cannot be "rotated" — it is fixed. | Redeploy (see below) |
...
1. **Rotate the program upgrade authority (`autara-deployer.key`) — DO FIRST.**
   ...
   `eee682c2…` to a **new, never-committed** keypair (generated into
   `autara-deploy/.keys-testnet/`). Until this is done, anyone with the leaked
   deployer key can upgrade the live program bytecode. After transfer, the old
   deployer key is inert for upgrades.
...
5. **Redeploy programs + mints to fresh addresses (preferred clean slate).**
   The existing `autara-deploy/scripts/autara.testnet.fresh.env` flow already
   deploys to brand-new program/oracle addresses using gitignored keys and a
   `sync-program-id.sh` + ELF rebuild. Standing up the fresh deployment and
   cutting clients over to it is the most thorough remediation, since the old
   program/mint **account addresses themselves cannot be rotated** — only their
   authorities can. Once traffic is migrated, the exposed `keys/` material is
   fully retired.

## Runtime after this change (no keys in the image)

The server image no longer contains any key bytes. `entrypoint.sh` decodes
secrets from base64 env vars at startup. The operator MUST provide:

- `SIGNER_KEY_B64` — server signer (or set `AUTARA_SIGNER_KEY` to a path).
- `TOKEN_AUTHORITY_KEY_B64` — token mint authority (referenced by `tokens.json`).
- `PROGRAM_KEY_B64`, `ORACLE_KEY_B64` — decoded so the server can derive the
  program/oracle ids at boot (the server reads these files when
  `--program-id`/`--oracle-program-id` are not passed).
- `TOKENS_JSON_B64` *(optional)* — overrides the baked-in `tokens.json`.

These should be set to **rotated** key material, not the exposed `keys/` bytes.
