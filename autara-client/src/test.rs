...
pub async fn deploy_program(
    ...
    if let Err(err) = ProgramDeployer::new(config)
        .try_deploy_program(
            path.to_string(),
            program_keypair,
            authority_keypair,
        ...
        .await
    {
        // 0.6.7's ProgramDeployerError has no TransactionError variant; keep
        // the "already exists" tolerance on the rendered message instead.
